...
- 设置、首次引导、选片弹窗和播放器开关统一使用 `MotionSwitch`。结构参考 shadcn/ui 的 Radix Switch：轨道用 `inline-flex + items-center`，默认几何为 40×22px / 18px 圆点 / 20px 位移，小号为 32×18px / 14px 圆点 / 16px 位移；业务页面不再覆写 padding、圆点尺寸或位移。首次渲染直接落在当前状态，状态切换只过渡 `transform`，并支持 reduced motion。
