...
追番中心把季初选番、日常追更、本地媒体管理、本地播放和观看统计接成一条完整路线。你可以把它当作私人放映厅的前台：作品资料从公开数据源补齐，观看进度与媒体文件留在自己的电脑里。
...
“刷新资料”会重新读取当前季度目录，并复核本地关联、Bangumi 剧集、长门身份、RSS 别名和中文简介。用户主动刷新时会等待长门番堂返回真实结果；季度相关页面依次读取，Bangumi 季度分页采用有界并发，降低上游突发失败。中文简介优先使用本地可信内容或标题、年份、季别都一致的豆瓣条目；没有可靠中文来源时继续显示 Bangumi 原文。长门番堂提供档期与制作情报，当前不提供单部剧情简介。

> **网络提示**
>
> - 长门番堂在国内可直接访问，无需开启代理或 VPN。
> - Bangumi 的资料、评分、评论、人物与关联信息需要开启代理或 VPN（俗称“魔法梯子”）获取。
> - 单集找源与剧集下载可直接进行，无需开启代理或 VPN；这条链路使用 用户自行配置的 RSS 与内置 qBittorrent。
...
| 用户自行配置的 RSS | 单集资源发现 | 可直连；用户自行选择来源并确认合法性 |
...
- 简体、繁体和原始标题变体。
- 绝对集号与季内集号，例如第四季的 `EP.74` 可以对应发布标题中的 `08`。
- 季别别名与篇章后缀。
- `SxxEyy` 季集格式。
...
不会。列表清理只移除追番中心的队列记录，qBittorrent 任务和本地文件继续保留。点击“刷新下载与资料”时，受管 qBittorrent 已标记为“文件缺失”的条目会从本地列表剔除，并记录来源抑制，防止同一失效任务再次回流。
...
长门番堂情报在界面与文档中保留来源链接和 [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/) 署名。Bangumi、AniList、TMDB、豆瓣以及各外部 RSS 来源均与本项目保持独立，其名称、数据与素材权利归各自权利人。

## 使用边界

追番中心不托管动画、电影或电视剧正片，也不随源码分发用户媒体文件。RSS、磁力链接、下载和播放功能只提供本地工具链；请自行确认内容来源、使用方式和所在地规则。项目与 Bangumi、长门番堂、AniList、TMDB、豆瓣、qBittorrent 及各内容发布方没有官方隶属或背书关系。
